...
trait ShouldMatchersForJUnit extends ShouldMatchers with AssertionsForJUnit {
  ...
  private[scalatest] override def newTestFailedException(message: String, optionalCause: Option[Throwable] = None, stackDepthAdjustment: Int): Throwable = {
    ...
    optionalCause match {
      case Some(cause) => new JUnitTestFailedError(message, cause, stackDepth + stackDepthAdjustment)
      case None => new JUnitTestFailedError(message, stackDepth + stackDepthAdjustment)
    }
  }
}
