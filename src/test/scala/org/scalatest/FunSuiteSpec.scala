...
/*
    ...
    it("should execute one test when run is called with a defined testName") {
      ...
      a.run(Some("this"), RunArgs(SilentReporter, new Stopper {}, Filter(), Map(), None, new Tracker, Set.empty))
    ...
    it("should exclude a test with a tag included in the tagsToExclude set even if run is invoked with that testName") {
      ...
      val repE = new TestIgnoredTrackingReporter
      e.run(Some("test this"), RunArgs(repE, new Stopper {}, Filter(None, Set("org.scalatest.SlowAsMolasses")), Map(), None, new Tracker, Set.empty))
      assert(!repE.testIgnoredReceived)
      assert(!e.theTestThisCalled)
      assert(!e.theTestThatCalled)
    }
    ...
    it("should throw IllegalArgumentException if passed a testName that doesn't exist") {
      ...
      intercept[IllegalArgumentException] {
        suite.run(Some("three"), RunArgs(SilentReporter, new Stopper {}, Filter(), Map(), None, new Tracker, Set.empty))
      }
    }
    ...
    it("should throw a NotAllowedException if chosenStyles is defined and does not include FunSuite") {
      ...
      simpleSuite.run(None, RunArgs(SilentReporter, new Stopper {}, Filter(), Map("org.scalatest.ChosenStyles" -> Set("org.scalatest.FunSuite")), None, new Tracker, Set.empty))
      val caught =
